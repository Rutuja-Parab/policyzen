    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                            <p class="text-sm text-blue-800">
                                ...
                                Premium will be calculated automatically based on the student's sum insured and coverage
                                period.
                            </p>
                        </div>
                        ...
                        <div class="flex items-center justify-between pt-4">
                            <button type="button" onclick="selectAllAvailable()"
                                class="text-sm text-[#f06e11] hover:text-blue-800">
                                Select All
                            </button>
                            <button type="submit"
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                ...
                <form action="{{ route('policies.students.remove', $policy) }}" method="POST"
                    ...
                    <div class="space-y-4">
                        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                            <p class="text-sm text-blue-800">
                                <i class="fas fa-info-circle mr-2"></i>
                                Refund will be calculated automatically based on the actual coverage period.
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="file" name="endorsement_documents[]" multiple
                                ...
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2b8bd0] focus:border-blue-500">
                            <p class="text-xs text-gray-500 mt-1">Upload supporting documents (PDF, DOC, DOCX, JPG, PNG -
                                Max 10MB each)</p>
                        </div>
                        ...
                        <div class="flex items-center justify-between pt-4">
                            <button type="button" onclick="selectAllAttached()"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200">
            <div class="p-6 border-b border-gray-200 flex justify-between items-center">
                ...
                <a href="{{ route('policies.students.audit-logs', $policy) }}"
                    class="text-[#f06e11] hover:text-blue-800 text-sm">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($auditLogs as $log)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 text-sm text-gray-900">{{ $log->created_at->format('d M Y H:i') }}
                                </td>
                                <td class="px-6 py-4">
                                    <span
                                ...
                                <td class="px-6 py-4 text-sm text-gray-900">{{ $log->metadata['student_name'] ?? 'N/A' }}
                                </td>
                                <td
                                    class="px-6 py-4 text-sm font-medium {{ $log->transaction_type === 'DEBIT' ? 'text-red-600' : 'text-green-600' }}">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-500">
                                    @if ($log->endorsement)
                                        <a href="{{ route('policies.students.endorsement.download', [$policy, $log->endorsement_id]) }}"
                                            ...
                                            <i
                                                class="fas fa-file-pdf mr-1"></i>{{ $log->metadata['endorsement_number'] ?? 'Download' }}
                                        </a>
                                    @else
                                        -
                                    @endif
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-8 text-center text-gray-500">No activity recorded yet
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <script>
            ...
            function selectAllAvailable() {
                document.querySelectorAll('input[name="student_ids[]"]').forEach((checkbox, index) => {
                    if (index < document.querySelectorAll('.bg-white:first-of-type input[name="student_ids[]"]')
                        .length) {

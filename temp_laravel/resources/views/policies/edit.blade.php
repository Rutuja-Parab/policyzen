...
@section('title', 'Edit Policy - PolicyZen')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="insurance_type" required
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-[#2b8bd0] focus:border-transparent">
                            <option value="HEALTH"
                                {{ old('insurance_type', $policy->insurance_type) == 'HEALTH' ? 'selected' : '' }}>Health
                                Insurance</option>
                            <option value="ACCIDENT"
                                {{ old('insurance_type', $policy->insurance_type) == 'ACCIDENT' ? 'selected' : '' }}>
                                Accident Insurance</option>
                            <option value="PROPERTY"
                                {{ old('insurance_type', $policy->insurance_type) == 'PROPERTY' ? 'selected' : '' }}>
                                Property Insurance</option>
                            <option value="VEHICLE"
                                {{ old('insurance_type', $policy->insurance_type) == 'VEHICLE' ? 'selected' : '' }}>Vehicle
                                Insurance</option>
                            <option value="MARINE"
                                {{ old('insurance_type', $policy->insurance_type) == 'MARINE' ? 'selected' : '' }}>Marine
                                Insurance</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="provider" required
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-[#2b8bd0] focus:border-transparent"
                            value="{{ old('provider', $policy->provider) }}">
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="starting_coverage_pool" required pattern="^[0-9]+(?:\.[0-9]{1,2})?$"
                                ...
                                value="{{ old('starting_coverage_pool', $policy->starting_coverage_pool) }}" maxlength="15"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="available_coverage_pool" required
                                pattern="^[0-9]+(?:\.[0-9]{1,2})?$"
                                class="w-full border border-gray-300 rounded-lg px-8 py-2 focus:ring-2 focus:ring-[#2b8bd0] focus:border-transparent"
                                value="{{ old('available_coverage_pool', $policy->available_coverage_pool) }}"
                                maxlength="15" oninput="formatCurrency(this)">
                        </div>
                        <p class="text-xs text-gray-500 mt-1">Enter amount in Indian Rupees (e.g., 100000.00 for ₹1,00,000)
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="utilized_coverage_pool" required pattern="^[0-9]+(?:\.[0-9]{1,2})?$"
                                class="w-full border border-gray-300 rounded-lg px-8 py-2 focus:ring-2 focus:ring-[#2b8bd0] focus:border-transparent"
                                value="{{ old('utilized_coverage_pool', $policy->utilized_coverage_pool) }}" maxlength="15"
                                oninput="formatCurrency(this)">
                        </div>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <p class="text-xs text-gray-500 mt-1">Supported formats: PDF, DOC, DOCX, JPG, JPEG, PNG (Max
                                10MB)</p>
                            @if ($policy->documents && count($policy->documents) > 0)
                                <div class="mt-2">
                                    <p class="text-sm text-gray-600">Current documents:</p>
                                    <ul class="mt-1 space-y-1">
                                        @foreach ($policy->documents as $doc)
                                            @if ($doc->type === 'policy_document')
                                                <li class="text-sm text-gray-500 flex items-center">
                                                    <i class="fas fa-file mr-2"></i>
                                                    <a href="{{ Storage::url($doc->file_path) }}" target="_blank"
                                                        class="text-[#2b8bd0] hover:underline">
                                                        {{ $doc->original_name }}
                                                    </a>
                                                </li>
                                            @endif
                        ...
                        <div>
                            ...
                            @if ($policy->documents && count($policy->documents) > 0)
                                <div class="mt-2">
                                    <p class="text-sm text-gray-600">Current supporting documents:</p>
                                    <ul class="mt-1 space-y-1">
                                        @foreach ($policy->documents as $doc)
                                            @if ($doc->type === 'supporting_document')
                                                <li class="text-sm text-gray-500 flex items-center">
                                                    <i class="fas fa-file mr-2"></i>
                                                    <a href="{{ Storage::url($doc->file_path) }}" target="_blank"
                                                        class="text-[#2b8bd0] hover:underline">

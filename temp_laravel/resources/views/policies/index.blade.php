...
@section('header-actions')
    <a href="{{ route('policies.create') }}" class="bg-[#f06e11] text-white px-4 py-2 rounded-lg hover:bg-[#f28e1f]">
...
@section('content')
    ...
    <!-- Search and Filters -->
    <x-search-filters action="{{ route('policies.index') }}"
        search-placeholder="Search policies by number, provider, type, or entity..." :date-range="true">
        <div>
    ...
    <div class="mb-4 flex items-center justify-between">
        <div class="text-sm text-gray-600">
            @if ($policies->total() > 0)
                Showing {{ $policies->firstItem() ?? 0 }} to {{ $policies->lastItem() ?? 0 }} of {{ $policies->total() }}
                policies
            @else
                No policies found
            @endif
        ...
        @if (request()->hasAny(['search', 'status', 'insurance_type', 'date_from', 'date_to', 'premium_min', 'premium_max']))
            <div class="text-sm">
                <a href="{{ route('policies.index') }}" class="text-[#f06e11] hover:text-blue-800">
    ...
    @if ($policies->count() > 0)
        ...
        <div class="mb-4 flex items-center space-x-4">
            <div class="flex items-center">
                ...
                <label for="selectAll" class="ml-2 text-sm text-gray-700">Select All ({{ $policies->total() }}
                    total)</label>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($policies as $policy)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                <td class="px-2 py-3 whitespace-nowrap">
                                    <input type="checkbox" name="selected_policies[]" value="{{ $policy->id }}"
                                        class="policy-checkbox rounded border-gray-300 text-[#f06e11] shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">{{ $policy->policy_number }}</div>
                                    @if ($policy->start_date && $policy->end_date)
                                        <div class="text-xs text-gray-500">{{ $policy->start_date }} to
                                            {{ $policy->end_date }}</div>
                                    @endif
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                            ...
                            @if ($policy->insurance_type == 'HEALTH') bg-[#f06e11]/10 text-[#f06e11]
                            @elseif($policy->insurance_type == 'ACCIDENT') bg-[#04315b]/10 text-[#04315b]
                            @elseif($policy->insurance_type == 'PROPERTY') bg-[#2b8bd0]/10 text-[#2b8bd0]
                            @elseif($policy->insurance_type == 'VEHICLE') bg-[#f28e1f]/10 text-[#f28e1f]
                                    ...
                                    @if ($policy->entities->count() > 0)
                                        <div class="text-xs text-gray-500 truncate max-w-24">
                                            @foreach ($policy->entities->take(2) as $entity)
                                                {{ $entity->description }}@if (!$loop->last)
                                                    ,
                                                @endif
                                            @endforeach
    ...
    <script>
        ...
        function performBulkAction() {
            ...
            let confirmMessage =
                `Are you sure you want to perform this action on ${selectedPolicies.length} selected policy(s)?`;

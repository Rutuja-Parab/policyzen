...
@section('header-actions')
    <a href="{{ route('entities.vehicles.create') }}" class="bg-[#f06e11] text-white px-4 py-2 rounded-lg hover:bg-[#f28e1f]">
...
@section('content')
    ...
    <!-- Search and Filters -->
    <x-search-filters action="{{ route('entities.vehicles.index') }}"
        search-placeholder="Search vehicles by registration number, make, model, or company..." :date-range="false">
        <div>
    ...
    <div class="mb-4 flex items-center justify-between">
        <div class="text-sm text-gray-600">
            @if ($vehicles->total() > 0)
                Showing {{ $vehicles->firstItem() ?? 0 }} to {{ $vehicles->lastItem() ?? 0 }} of {{ $vehicles->total() }}
                vehicles
            @else
                No vehicles found
            @endif
        ...
        @if (request()->hasAny(['search', 'status', 'company_id', 'make', 'year_min', 'year_max']))
            <div class="text-sm">
                <a href="{{ route('entities.vehicles.index') }}" class="text-[#f06e11] hover:text-blue-800">
    ...
    @if ($vehicles->count() > 0)
        ...
        <div class="mb-4 flex items-center space-x-4">
            <div class="flex items-center">
                ...
                <label for="selectAll" class="ml-2 text-sm text-gray-700">Select All ({{ $vehicles->total() }}
                    total)</label>
            </div>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <input type="checkbox" name="selected_vehicles[]" value="{{ $vehicle->id }}"
                                        class="vehicle-checkbox rounded border-gray-300 text-[#f06e11] shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">{{ $vehicle->registration_number }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            @else bg-red-100 text-red-800 @endif">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        ...
                                        <form method="POST" action="{{ route('entities.vehicles.destroy', $vehicle) }}"
                                            class="inline"
    ...
    <script>
        ...
        function performBulkAction() {
            ...
            let confirmMessage =
                `Are you sure you want to perform this action on ${selectedVehicles.length} selected vehicle(s)?`;

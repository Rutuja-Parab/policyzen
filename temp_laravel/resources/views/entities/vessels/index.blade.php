...
@section('title', 'Vessels - PolicyZen')
...
<!-- Search and Filters -->
<x-search-filters 
    action="{{ route('entities.vessels.index') }}"
    search-placeholder="Search vessels by name, IMO number, type, flag state, or company..."
    :date-range="false"
>
    <div>
...
<div class="mb-4 flex items-center justify-between">
    <div class="text-sm text-gray-600">
        @if($vessels->total() > 0)
        Showing {{ $vessels->firstItem() ?? 0 }} to {{ $vessels->lastItem() ?? 0 }} of {{ $vessels->total() }} vessels
        @else
        No vessels found
        @endif
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        <a href="{{ request()->url() }}?{{ http_build_query(array_merge(request()->query(), ['sort_by' => 'vessel_name', 'sort_order' => request('sort_order') == 'asc' ? 'desc' : 'asc'])) }}" 
                           class="hover:text-gray-700">
                            Vessel Name
                            @if(request('sort_by') == 'vessel_name')
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        <a href="{{ request()->url() }}?{{ http_build_query(array_merge(request()->query(), ['sort_by' => 'imo_number', 'sort_order' => request('sort_order') == 'asc' ? 'desc' : 'asc'])) }}" 
                           class="hover:text-gray-700">
                            IMO Number
                            @if(request('sort_by') == 'imo_number')
                                <i class="fas fa-sort-{{ request('sort_order') == 'asc' ? 'up' : 'down' }} ml-1"></i>
                            @else
                                <i class="fas fa-sort ml-1 opacity-50"></i>
                            @endif
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Flag State</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition-colors duration-150">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-900">{{ $vessel->imo_number }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-900">{{ $vessel->vessel_type ?? 'N/A' }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-900">{{ $vessel->flag_state ?? 'N/A' }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-900">{{ $vessel->company->name ?? 'N/A' }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div class="flex items-center justify-end space-x-2">
                            ...
                            <a href="{{ route('entities.vessels.edit', $vessel) }}" 
                               class="text-green-600 hover:text-green-900 p-1 rounded" 
                               title="Edit">
                                <i class="fas fa-edit"></i>
                            </a>
                            <form method="POST" action="{{ route('entities.vessels.destroy', $vessel) }}" class="inline" 
                                  onsubmit="return confirm('Are you sure you want to delete this vessel?')">
                                @csrf
                                @method('DELETE')
...
<div class="mt-6">
    <div class="flex items-center justify-between">
        <div class="text-sm text-gray-700">
            Page {{ $vessels->currentPage() }} of {{ $vessels->lastPage() }}
        </div>
        <div class="flex items-center space-x-1">
            {{ $vessels->links('pagination::tailwind') }}
        </div>
    </div>
</div>
@endif
...
<div class="text-center py-12">
    ...
    <p class="text-gray-500 mb-6">
        @if(request()->hasAny(['search', 'status', 'company_id', 'vessel_type', 'flag_state']))
            No vessels match your current filters. Try adjusting your search criteria.
        @else
    ...
    <div class="flex justify-center space-x-4">
        @if(request()->hasAny(['search', 'status', 'company_id', 'vessel_type', 'flag_state']))
        <a href="{{ route('entities.vessels.index') }}" class="bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-gray-700">
...
function performBulkAction() {
    const selectedVessels = Array.from(document.querySelectorAll('.vessel-checkbox:checked')).map(cb => cb.value);
    ...
    let confirmMessage = `Are you sure you want to perform this action on ${selectedVessels.length} selected vessel(s)?`;
    ...
    form.method = 'POST';
    form.action = '{{ route("entities.vessels.bulk-action") }}';

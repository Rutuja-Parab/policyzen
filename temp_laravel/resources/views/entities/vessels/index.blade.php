        ...
        <div class="flex-1">
            ...
            <main class="p-6">
                ...
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-500 mb-4">Start by adding your first vessel</p>
                    <a href="{{ route('entities.vessels.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
                        <i class="fas fa-plus mr-2"></i>Add Vessel
                    </a>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div class="bg-white rounded-lg shadow hover:shadow-lg transition-shadow duration-200">
                        <div class="p-6">
                            ...
                            <div class="space-y-2">
                                <h3 class="font-semibold text-gray-900 text-lg">{{ $vessel->vessel_name }}</h3>
                                <p class="text-sm text-gray-600">IMO: {{ $vessel->imo_number }}</p>
                                @if($vessel->vessel_type)
                                <p class="text-sm text-gray-500">{{ $vessel->vessel_type }}</p>
                                @endif
                                @if($vessel->flag)
                                <p class="text-sm text-gray-500">Flag: {{ $vessel->flag }}</p>
                                @endif
                            </div>

                            <div class="flex space-x-2 mt-4 pt-4 border-t border-gray-100">
                                <a href="{{ route('entities.vessels.show', $vessel) }}" class="flex-1 bg-gray-100 text-gray-700 px-3 py-2 rounded-lg hover:bg-gray-200 text-center text-sm">
                                    <i class="fas fa-eye mr-1"></i>View
                                </a>
                                <a href="{{ route('entities.vessels.edit', $vessel) }}" class="flex-1 bg-blue-600 text-white px-3 py-2 rounded-lg hover:bg-blue-700 text-center text-sm">

...
@section('content')
    ...
    <!-- Search and Filters -->
    <x-search-filters action="{{ route('entities.vessels.index') }}"
        search-placeholder="Search vessels by name, IMO number, type, flag state, or company..." :date-range="false">
        <div>
    ...
    <div class="mb-4 flex items-center justify-between">
        <div class="text-sm text-gray-600">
            @if ($vessels->total() > 0)
                Showing {{ $vessels->firstItem() ?? 0 }} to {{ $vessels->lastItem() ?? 0 }} of {{ $vessels->total() }}
                vessels
            @else
                No vessels found
            @endif
    ...
    @if ($vessels->count() > 0)
        ...
        <div class="mb-4 flex items-center space-x-4">
            <div class="flex items-center">
                ...
                <label for="selectAll" class="ml-2 text-sm text-gray-700">Select All ({{ $vessels->total() }}
                    total)</label>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Flag
                                State</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Company</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            ...
                            @else bg-red-100 text-red-800 @endif">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        ...
                                        <form method="POST" action="{{ route('entities.vessels.destroy', $vessel) }}"
                                            class="inline"
    ...
    <script>
        ...
        function performBulkAction() {
            ...
            let confirmMessage =
                `Are you sure you want to perform this action on ${selectedVessels.length} selected vessel(s)?`;

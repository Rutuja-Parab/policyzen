...
@section('title', 'Students - PolicyZen')
...
<!-- Search and Filters -->
<x-search-filters 
    action="{{ route('entities.students.index') }}"
    search-placeholder="Search students by name, ID, email, rank, or company..."
    :date-range="false"
>
    <div>
    ...
    <div>
        ...
        <select id="course_id" name="course_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
...
<div class="mb-4 flex items-center space-x-4">
    <div class="flex items-center">
        ...
        <label for="selectAll" class="ml-2 text-sm text-gray-700">Select All ({{ $students->total() }} total)</label>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        <a href="{{ request()->url() }}?{{ http_build_query(array_merge(request()->query(), ['sort_by' => 'age', 'sort_order' => request('sort_order') == 'asc' ? 'desc' : 'asc'])) }}" 
                           class="hover:text-gray-700">
                            Age/Gender
                    ...
                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Rank/Course</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition-colors duration-150">
                    ...
                    <td class="px-3 py-3 whitespace-nowrap">
                        ...
                        <div class="text-xs text-gray-500 truncate max-w-24">{{ $student->course ?? 'N/A' }}</div>
                    </td>
                    <td class="px-3 py-3 whitespace-nowrap">
                    ...
                    <td class="px-2 py-3 whitespace-nowrap text-right text-sm font-medium">
                        <div class="flex items-center justify-end space-x-1">
                            ...
                            <form method="POST" action="{{ route('entities.students.destroy', $student) }}" class="inline" 
...
<div class="text-center py-12">
    ...
    <div class="flex justify-center space-x-4">
        @if(request()->hasAny(['search', 'status', 'company_id', 'course_id', 'gender', 'age_min', 'age_max']))
        <a href="{{ route('entities.students.index') }}" class="bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-gray-700">
...
function performBulkAction() {
    ...
    if (action === 'export') {
        exportSelectedStudents(selectedStudents);
        return;
    }
    
    let confirmMessage = `Are you sure you want to perform this action on ${selectedStudents.length} selected student(s)?`;
    ...
    form.method = 'POST';
    form.action = '{{ route("entities.students.bulk-action") }}';

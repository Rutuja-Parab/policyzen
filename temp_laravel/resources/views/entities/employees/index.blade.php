...
@section('content')
    ...
    <!-- Search and Filters -->
    <x-search-filters action="{{ route('entities.employees.index') }}"
        search-placeholder="Search employees by name, code, department, or company..." :date-range="false">
        <div>
    ...
    <div class="mb-4 flex items-center justify-between">
        ...
        @if (request()->hasAny(['search', 'status', 'company_id', 'department']))
            <div class="text-sm">
                <a href="{{ route('entities.employees.index') }}" class="text-[#f06e11] hover:text-blue-800">
    ...
    @if ($employees->count() > 0)
        ...
        <div class="mb-4 flex items-center space-x-4">
            <div class="flex items-center">
                ...
                <label for="selectAll" class="ml-2 text-sm text-gray-700">Select All ({{ $employees->total() }}
                    total)</label>
            </div>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <input type="checkbox" name="selected_employees[]" value="{{ $employee->id }}"
                                        class="employee-checkbox rounded border-gray-300 text-[#f06e11] shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            @else bg-red-100 text-red-800 @endif">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        ...
                                        <form method="POST"
                                            action="{{ route('entities.employees.destroy', $employee) }}" class="inline"
    ...
    <script>
        ...
        function performBulkAction() {
            const selectedEmployees = Array.from(document.querySelectorAll('.employee-checkbox:checked')).map(cb => cb
                .value);
            ...
            let confirmMessage =
                `Are you sure you want to perform this action on ${selectedEmployees.length} selected employee(s)?`;

    ...
    <div class="flex items-center space-x-3">
        @if ($unreadCount > 0)
            <form method="POST" action="{{ route('notifications.mark-all-read') }}" class="inline">
                @csrf
                <button type="submit"
                    class="px-4 py-2 bg-[#f06e11] text-white rounded-lg hover:bg-[#f28e1f] transition-colors text-sm font-medium">
    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div class="bg-white rounded-lg shadow p-6">
                <div class="flex items-center">
                    <div class="flex-shrink-0">
                        <i class="fas fa-bell text-2xl text-[#f06e11]"></i>
                    </div>
                    <div class="ml-4">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <div class="flex items-center">
                    <div class="flex-shrink-0">
                        <i class="fas fa-envelope text-2xl text-[#f06e11]"></i>
                    </div>
                    <div class="ml-4">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <div class="flex items-center">
                    <div class="flex-shrink-0">
                        <i class="fas fa-exclamation-triangle text-2xl text-[#04315b]"></i>
                    </div>
                    <div class="ml-4">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <div class="flex items-center">
                    <div class="flex-shrink-0">
                        <i class="fas fa-calendar text-2xl text-[#2b8bd0]"></i>
                    </div>
                    <div class="ml-4">
            ...
            <form method="GET" class="flex flex-wrap items-center gap-4">
                ...
                <div>
                    ...
                    <select name="type" id="type" class="border border-gray-300 rounded-lg px-3 py-2 text-sm">
                        <option value="">All</option>
                        <option value="POLICY_EXPIRY_WARNING"
                            {{ request('type') === 'POLICY_EXPIRY_WARNING' ? 'selected' : '' }}>Policy Expiry</option>
                        <option value="ENDORSEMENT_PENDING"
                            {{ request('type') === 'ENDORSEMENT_PENDING' ? 'selected' : '' }}>Endorsement</option>
                        <option value="POLICY_EXPIRED" {{ request('type') === 'POLICY_EXPIRED' ? 'selected' : '' }}>Policy
                            Expired</option>
                    </select>
                </div>
            ...
            @if ($notifications->count() > 0)
                <div class="divide-y divide-gray-200">
                    @foreach ($notifications as $notification)
                        <div
                            class="p-6 hover:bg-gray-50 transition-colors {{ !$notification->isRead() ? 'bg-[#04315b]/5 border-l-4 border-[#f06e11]' : '' }}">
                            <div class="flex items-start space-x-4">
                                ...
                                <div class="flex-shrink-0">
                                    ...
                                    @elseif($notification->priority === 'HIGH') bg-[#f06e11]/10 text-[#f06e11]
                                    @elseif($notification->priority === 'MEDIUM') bg-[#2b8bd0]/10 text-[#2b8bd0]
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center justify-between">
                                        <h3
                                            class="text-lg font-semibold text-gray-900 {{ !$notification->isRead() ? 'font-bold' : '' }}">
                                            {{ $notification->title }}
                                        </h3>
                                        <div class="flex items-center space-x-2">
                                            ...
                                            @if (!$notification->isRead())
                                                <span class="w-2 h-2 bg-[#f06e11] rounded-full"></span>
                                            @endif
                                        </div>
                                    </div>
                                    ...
                                    <div class="mt-3 flex items-center justify-between">
                                        ...
                                        <div class="flex items-center space-x-2">
                                            @if ($notification->isRead())
                                                <form method="POST"
                                                    action="{{ route('notifications.mark-unread', $notification) }}"
                                                    class="inline">
                                                    @csrf
                                                    @method('POST')
                                                    <button type="submit"
                ...
                <div class="p-12 text-center">
                    ...
                    <p class="text-gray-500">
                        @if (request()->anyFilled(['status', 'priority', 'type', 'search']))
                            ...
                            <a href="{{ route('notifications.index') }}" class="text-[#f06e11] hover:text-blue-800">clear
                                all filters</a>.
                        @else
